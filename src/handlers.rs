...
///
pub fn api_upload_init(ctx: Ctx, info: UploadInitPost) -> impl Future<Item=impl warp::Reply, Error=warp::Rejection> {
    let info = futures::future::result::<UploadInit, error::Error>(|| -> error::Result<UploadInit> {
        let info = info.decode_hex()
            .map_err(|_| error::helpers::bad_request("malformed info"))?;
        if info.size > models::CONFIG.upload_limit_bytes {
            return Err(
                    error::helpers::too_large(
                        format!("Upload too large, max bytes: {}", models::CONFIG.upload_limit_bytes)));
        }
        Ok(info)
    }());
    ...
    info.and_then(move |info: UploadInit| {
        cpu.spawn_fn(move || -> error::Result<String> {
            ...
            let access_auth = models::NewAuth::from_pass_bytes(&info.access_password)?
                .insert(&trans)?;
            let deletion_auth = match info.deletion_password {
                Some(ref bytes) => {
                    let auth = models::NewAuth::from_pass_bytes(bytes)?
                        .insert(&trans)?;
                    Some(auth.id)
                }
                None => None,
...
pub fn api_upload_file(ctx: Ctx, upload_key: UploadKey, body: warp::body::BodyStream) -> impl Future<Item=impl warp::Reply, Error=warp::Rejection> {
    struct Info {
        uuid: Uuid,
        now: DateTime<Utc>,
    ...
    info.and_then(move |mut info: Info| {
        cpu_create.spawn_fn(move || -> error::Result<Info> {
            ...
            if ! models::Status::can_fit(&trans, init_upload.size)? {
                return Err(
                    error::helpers::out_of_space("Server out of storage space")
                );
            }
            ...
            if ! init_upload.still_valid(&info.now) {
                return Err(
                    error::helpers::bad_request("Upload request came too late")
                );
            }
    ...
    .map_err(|e| (e, None))
    .and_then(move |info: Info| { // -> `impl Future<Item=usize, Error=(error::Error, Option<models::Upload>)>`
        use warp::Stream;
        use warp::Buf;
                ...
                body
                    ...
                    .fold((file, 0), move |(file, byte_count), buf| {
                        futures::future::result::<Vec<u8>, (error::Error, Option<models::Upload>)>({
                            ...
                            if size > max_bytes {
                                Err((error::helpers::too_large("upload too large"),
                                    Some(upload.clone())))
                            } else {
                                Ok(bytes.to_vec())
                            }
                            ...
                            tokio::io::write_all(file, bytes)
                                .map(move |(file, bytes_written)| (file, byte_count + bytes_written.len()))
                                .map_err(|e| (error::Error::from(e), None))
                        })
                    }).map(|(_file, size)| size)
            })
        ...
        cpu_delete.spawn_fn(move || -> error::Result<String> {
            match err.kind() {
                error::ErrorKind::UploadTooLarge(_) => {
                    match maybe_upload {
                        Some(upload) => {
            ...
            tokio::fs::remove_file(path)
                ...
                .and_then(|_| futures::future::err(error::helpers::too_large("upload too large")))
        })
...
pub fn api_upload_delete(ctx: Ctx, delete_key: DeleteKeyAccessPost) -> impl Future<Item=impl warp::Reply, Error=warp::Rejection> {
    let cpu = ctx.cpu;
    let db = ctx.db;
    futures::future::result::<DeleteKeyAccess, error::Error>(delete_key.decode_hex())
        ...
        .and_then(|file_path| {
            tokio::fs::remove_file(file_path.clone())
                .or_else(move |e| {
                    error!("Error deleting upload file {}, {} continuing...", file_path, e);
                    futures::future::ok(())
                })
        })
        .map(|_| {
...
pub fn api_download_init(ctx: Ctx, download_key: DownloadKeyAccessPost) -> impl Future<Item=impl warp::Reply, Error=warp::Rejection> {
    struct Info {
        now: DateTime<Utc>,
    ...
    futures::future::result::<Info, error::Error>((|| {
        ...
        cpu.spawn_fn(move || -> error::Result<Data> {
            ...
            Ok(Data {
                upload: upload, init_download_content, init_download_confirm
            })
        })
    })
    .map(|data| {
...
pub fn api_download(ctx: Ctx, download_key: DownloadKeyAccessPost) -> impl Future<Item=impl warp::Reply, Error=warp::Rejection> {
    struct Info {
        now: DateTime<Utc>,
        download_key: DownloadKeyAccess,
    }
    ...
    futures::future::result::<Info, error::Error>((|| {
        ...
        cpu.spawn_fn(move || -> error::Result<models::Upload> {
            ...
            let init_download = models::InitDownload::find(&trans, &info.download_key.uuid, models::DownloadType::Content)?;
...
pub fn api_download_confirm(ctx: Ctx, download_key: DownloadKeyHash) -> impl Future<Item=impl warp::Reply, Error=warp::Rejection> {
    struct Info {
        hash_bytes: Vec<u8>,
        uuid: Uuid,
    }
    ...
    futures::future::result::<Info, error::Error>((|| {
        ...
        cpu.spawn_fn(move || -> error::Result<models::Upload> {
            ...
            let init_download = models::InitDownload::find(&*conn, &info.uuid, models::DownloadType::Confirm)?;
            let upload = init_download.get_upload(&trans)?;
